  ...
  function addlinha(dadosAPI){
    ...
    function cadastrar(){
      ...
      const email = document.getElementById('descricao').value;
      if(nome && email){
 
        this.addlinha([{"nome":nome.trim(), "descricao":email.trim()}]);
        ...
        fetch('http://localhost:8080/api/cargos', { 
          ...
          body: JSON.stringify({"nome":nome, "descricao":email})
        })
        .then(response => response.json())
        .then(data => {
  ...
  const descricaoAtual = linha.children[2].innerText;

  Swal.fire({
    title: 'Editar dados',
    html: `
      <input id="editNome" class="swal2-input" placeholder="nome" value="${nomeAtual}">
      <input id="editdescricao" class="swal2-input" placeholder="descricao" value="${descricaoAtual}">
    `,
    ...
    if (result.isConfirmed) {
      ...
      .then(() => {
        ...
        linha.children[2].innerText = result.value.descricao;
